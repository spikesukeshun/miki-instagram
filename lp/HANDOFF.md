...
- **右端のセクションレール（`.rail`）はなぞって移動できる**。以前はタップのみ。
  ...
  - **区間が切り替わるたびに触覚フィードバックを鳴らす（`haptic()` / 2026-08-13 追加）**。
    iOS Safari は **Vibration API を実装していない**（`navigator.vibrate` が無い）。
    iOS 17.4 で入った `<input type="checkbox" switch>` は、ラベルを押すとシステムの触覚が鳴るので
    その副作用を借りている。Android は `navigator.vibrate(8)`。
    - ⚠ **正式なAPIではない**。Appleが塞いだら鳴らなくなるが、見た目と動作は変わらない作りにしてある。
    - ⚠ **隠しラベルは `position:fixed` でなければならない**。`absolute` だとラベルのクリックで
      チェックボックスにフォーカスが移り、ブラウザがそれを画面内へ入れようとして
      **ページが先頭までスクロールする**（実測 scrollY 9000 → 0）。fixed なら常に画面内なので起きない。
    - クリック後は元の要素へ `focus({preventScroll:true})` でフォーカスを戻している。
    - 鳴らすのは `railPreview()` の中だけ。同じ区間では早期returnするので連打にならない。
