...
- **スマホは一覧を小さくして、読むのは拡大側に寄せている（2026-08-12 再修正）**。
  ...
  「拡大した意味がない」**状態だった。そこで
  `@media (max-width:599px){.carousel{max-width:300px}}` で一覧を280pxまで落とし、
  さらに**600px未満では拡大表示を最初から `zoomed` で開く**（`openReview` の `startZoomed`）。
...
- **スマホの本文は約10.5pxにしかならない**。375px画面では画像幅315px＝等倍の0.82倍が上限で、
  ...
  - さらに `#lbZoom`「文字を大きく」で `width:min(175%,660px)`。375px画面で本文の行送りが
    ...
    660pxの上限が要る。** 600px以上の画面は等倍表示のままで十分読めるのでボタンごと隠す。
...
- **右端のセクションレール（`.rail`）はなぞって移動できる**。以前はタップのみ。
  ...
  - `pointermove` のたびに指の下のドットを `getBoundingClientRect` で判定し、変わったらその区間へ飛ぶ。
