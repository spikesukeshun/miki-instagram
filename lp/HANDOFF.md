...
- **プロフィール写真は挿入済み（2026-08-13）**。Drive `menu` フォルダの `IMG_9115.jpg`
  （**ID `1AjM2E8LFm1eWAe_YSRLmolkurZiUdFx5` / 2026-08-13 アップの方**）。
  ⚠ **同フォルダに同名 `IMG_9115.jpg` が2枚ある**（`1OCYjIPcPrR0dtkwUyAJdii6pwIElJfpo` は 08-08 アップ）。
  中身は別物で、**古い方は赤い花柄の寝具が写り込んでいる**。以前この写真を見送った理由がそこだったので、
  必ず**新しい方（花柄を処理済み）**を使うこと。名前で引くと取り違えるのでIDで指定する。
  - 3072x3064 の正方形を **4:5（中央基準で左右を落とす）→ 860x1075 / WebP q82（66KB）** で埋め込み。
  - `.ph::before` のプレースホルダのグラデーションを `.ph-miki::before` の `background-image` で上書きする方式。
    `.ph-photo` が `background-size:cover` を与える。**kenburns アニメーションはそのまま効かせている**（元々写真用の演出）。
  - 袖に小さな刺繍ロゴ（"AM…"）が写っているが、表示サイズでは判読不能。
...
| VIPコース | `.../reserve/?storeId=…&staffId=W000048120&couponId=CP00000009971204&add=0&rootCd=10` |
| スペシャルコース | `…&couponId=CP00000008015550&add=0&rootCd=10` |
| インディバ×首肩コリ | `…&couponId=CP00000012928170&add=0&rootCd=10` |
| バースデー特典 | `…&couponId=CP00000010821510&add=0&rootCd=10` |
| 星座別アロマ | `…&couponId=CP00000012584055&add=0&rootCd=10` |

### ⚠ `afterCoupon` を使わないこと（2026-08-15 修正）

以前は `/CSP/kr/reserve/afterCoupon?storeId=…&couponId=…&add=0` を使っていたが、
**これはホットペッパー側の予約セッションを前提とした「フローの途中」のURL**で、
Cookieを持たない人が外から踏むと **「予約エラー（Cookieは有効になっていますか？）」** になる。

自分のスマホでは通るのに他人に送ると通らない、という形で発覚した。
⚠ **Instagram のアプリ内ブラウザは Cookie が隔離されている**ため、
主要な流入層がほぼ確実に踏んでいた。

正しいのは **`afterCoupon` を外し、`staffId` と `rootCd=10` を足した形**。
これはホットペッパーのクーポン一覧ページ
（`https://beauty.hotpepper.jp/kr/slnH000075398/coupon/` ← **`/kr/` が必要**）の
「このクーポンで予約」ボタンが実際に使っているURLで、Cookie無しでも

- クーポン選択済み（「選択済みクーポン・メニュー」に表示される）
- MIKI指名済み（「選択中のスタッフ ／ クシヤ MIKI」と表示される）

の状態で**ご来店日時選択まで直行する**。5コースすべてで実測確認済み。
...
- **表示サイズは「等倍を超えない」で決めている（2026-08-12 修正）**。元スクショは402ptの画面を3倍で
  撮ったものなので **1158 / 3 = 386px が等倍**。`.carousel{max-width:414px}`（386＋左右パディング14px）。
  初版は760pxで出していて**等倍の1.89倍**になり、文字だけ巨大なうえ最長の口コミが1090pxと画面に
  収まらなかった（ユーザーから「デカすぎて見にくい」）。**ここを広げると同じ状態に戻る。**
- **スマホは一覧を小さくして、読むのは拡大側に寄せている（2026-08-12 再修正）**。
  375px画面では拡大表示の「等倍」も画面幅で頭打ちになるため、一覧315px→拡大334pxで**差が6%しかなく
  「拡大した意味がない」**状態だった。そこで
  `@media (max-width:599px){.carousel{max-width:300px}}` で一覧を280pxまで落とした。
  結果、タップで **280px → 584px（2.09倍・行送り13.1px→27.2px）** と一段ではっきり変わる。
  一覧側だけ小さくしても拡大幅は変わらないので、**この2つはセット**で意味を持つ。
- **拡大表示は必ず「全体を表示」の状態で開く（2026-08-13 ユーザー指定）**。
  一時期 600px未満だけ最初から `zoomed` で開いていたが、**どの口コミを見ているのか分からないまま
  本文の途中から始まる**ため差し戻した。拡大は `#lbZoom` を押したときだけ。
- **スマホの本文は約10.5pxにしかならない**。375px画面では画像幅315px＝等倍の0.82倍が上限で、
  これ以上大きくできない（＝小さくすると読めなくなる）。その代わりに**拡大表示**を用意している：
  - `.vcard img` をタップ／クリック → `.lb-box.is-review`（`overflow:auto` のスクロール可能な白シート）
  - さらに `#lbZoom`「文字を大きく」で `width:min(175%,660px)`。375px画面で本文の行送りが
    14.7px → 27.2px になる（横スクロールと引き換え）。**175%だけだと広い画面で2.6倍まで伸びるので
    660pxの上限が要る。**
  - **600px以上でも同じボタンを出す（2026-08-13 ユーザー指定）**。ただし広い画面で横に振らせるのは
    不自然なので、画像ではなく**カードごと広げる**：`@media (min-width:600px)` で
    `.lb-box.is-review.zoomed{width:min(980px,96vw)}` ＋ 画像は `width:100%`。
    1280px画面で **583px → 921px**、横スクロールなし。**ここを画像側の拡大に戻すと横スクロールが復活する。**
  - スワイプで送った直後に拡大が開かないよう、`pointerup` で**移動量10px未満のときだけ**タップ扱いにする。
  - スマホの `.lb.is-review-open` の上下パディングは閉じるボタン・拡大ボタンの領域。詰めるとカードの角に重なる。
- **既存のライトボックス（`[data-zoom]`）は流用していない**。`.lb-box` は `min(640px,92vw)`・`overflow:hidden`
  固定で縦1700pxの画像を入れると破綻するため、`is-review` を別モードとして足してある。
...
### 4-4. スマホの操作まわり（2026-08-12）

- **右端のセクションレール（`.rail`）はなぞって移動できる**。以前はタップのみ。
  - `.rail{touch-action:none}` が**必須**。これが無いと指の縦移動をページのスクロールに取られ、
    `pointermove` が来ないので「タップでしか動かない」状態に戻る。
  - `pointermove` のたびに指の下のドットを `getBoundingClientRect` で判定する。
  - **なぞっている間はページを動かさない（2026-08-13 修正）**。ドットとラベルだけを切り替え（`railPreview`）、
    実際の移動（`railGo`）は指を離したとき一度だけ。なぞるたびにスクロールしていたときは
    **下の画面が指に追従してガタガタ動いていた**（ユーザー報告「ブルブルする」）。
    あわせて、なぞっている間は `railUpdate`（スクロール追従）を止める。でないと指で選んだ区間を
    スクロール位置が上書きしてしまう。`pointercancel` は移動せずドットを実位置に戻す。
  - `railItems` の `click` は `preventDefault()` している。`href` のジャンプと二重に動くため。
  - **指を置いている間は区間名を画面中央に大きく出す（`.rail-hud` / 2026-08-13）**。
    ドット脇の `.rail-label` は**親指の下に入って読めない**（ユーザー報告）。
    レールは画面右端にあり、ラベルはドットのすぐ左に出るため、指と必ず重なる。
    - `pointerdown` / `pointermove` で `railHudShow()`、離したら `railHudHide()`。
    - HUD表示中は `.rail.touching` でドット脇のラベルを畳む（二重表示を避ける）。
    - **スクロールで切り替わったときは従来どおりドット脇の小さいラベル**。
      指が画面に無いので隠れず、こちらの方が邪魔にならない。
    - 実測（375px）：HUDは中央 x=188 / 文字21.4px（小ラベルの約1.8倍）、
      右端は273pxで画面右端から102px空く＝親指にかからない。
  - **触覚フィードバック（`navigator.vibrate` / iOS17.4の `<input switch>` ハック）は
    2026-08-13 に入れたが、実機で鳴らなかったのでユーザー判断により撤去した。**
    再挑戦する場合、隠しラベルを `position:absolute` にすると
    クリックでフォーカスが移り**ページが先頭までスクロールする**（実測 9000 → 0）。`fixed` にすること。
- **レールからの移動はスムーススクロールを使わない**（`behavior:"auto"`）。
  ページが約15,000pxあり、端から端へのスムーススクロールは**その間ずっと reveal・パララックス・
  ヘッダー更新が走る**ので一瞬固まる。ユーザー報告の「挙動が重い」はこれ。
  `html{scroll-behavior:smooth}` 自体は他のアンカーのために残してある。
- **パララックスはヒーローを過ぎたら書き換えを止める**（`parallaxLimit` / `parallaxIdle`）。
  以前はページ最下部でも `translate3d(0,2340px,0)` を毎フレーム書いていた。実測で
  scrollY=6000 と 13000 の変換行列が同じ（110.34px）になることを確認済み。
- **`.lb-close` には `z-index:2` が要る**。`.lb-box` は `transform` を持つのでDOM順で閉じるボタンより
  後に描画され、**モーダルが縦いっぱいになる短い画面でボタンが裏に隠れる**。背景も
  `rgba(30,26,22,.55)`＋blur に変えて、写真に重なっても見えるようにした。
- **`.ms-photo` はスマホだけ `aspect-ratio:2/1`**（1画面に収めるため）。元画像は 900x563＝ほぼ16:10 なので
  上下が2割トリミングされる。`m-s3`（ヘッドマッサージ）は中央basisだと**頭頂部が切れる**ので
  `.ms-photo.m-s3-img{background-position:center 15%}` で上寄せ。
  **`m-s4`（アフタータイム）だけ元画像が縦長**：Drive `menu` の `IMG_0155.jpg`
...
### 4-5. コースカードの写真 / 資格 / 操作性（2026-08-13）

- **コースカード右上の施術写真**（`.course-photo`）。スマホ112px / **PC 224px**（`@media (min-width:900px)`／ユーザー確定）。
  **PCで線形4倍（448px）は入らない。** カード幅642pxのうち、テキストの右側が空くのは価格ピルの手前まで＝
  上から約360px分。実測した「インクの右端」は上から順に 152→135→226→122→263→211 と来て、
  y=400 の価格ピルで 525 まで伸びる。**320pxが入る最大**だが、価格まわりと視線がぶつかって写真が主張しすぎるため224px（面積4倍）を採用した。
  - 元画像は Drive `menu`：VIP `4886701835173478510.177ddc…`（右側をカット）/
    スペシャル `…3e52424516…`（左右をカット）。640px角・WebP q80 で埋め込み（各24〜27KB）。
  - **選択チェック `.selglow` を右上→右下に移した**。写真と重なるため。戻すと写真に隠れる。
- **プロフィールに保有資格 `.p-cert`** を追加。主資格＋タグ＋「ほか多数」。
- **プロフィール写真を明るく補正**（ガンマ1.26・彩度1.05）。白飛び0%。元は平均輝度122.6→137.6。
- **ヒーローの「メニューを見る」を `.btn-ghost.btn-sm` に**。文字リンクだと背景に溶けて見えなかった。
- **DM下書きの手順を明示**（`.tmpl-steps`）。コピー後にDMが開くだけでは貼り付けに気づかれない。
- **口コミカルーセルのスワイプを作り直した**。以前は「反応が鈍い・たまに縦に動く」状態だった。
  - **`.carousel-track{touch-action:pan-y}` が必須**。無いとブラウザが横の動きもスクロール判定に使い、
    `pointercancel` でスワイプが消える。
  - 指にカードを追従させる（追従が無いと閾値を超えるまで無反応で「鈍い」）。
  - 最初の6pxで縦か横かを決め、縦と判定したらそのジェスチャは一切横に動かさない。
  - 送る条件は「幅の12%」または「速いフリック（>0.35px/ms かつ 12px超）」。距離だけだと速い操作を取りこぼす。

### スクリーンショットの撮り方（重要・2026-08-13 更新）

- **`/Applications/Google Chrome.app` のバイナリを使わないこと。**
  `--user-data-dir` を渡さないと起動中のユーザーのChromeに繋がり**新しいウィンドウが開く**（ユーザーから指摘あり）。
  渡すと今度は別プロセスとして起動できず `Abort trap: 6`（"Trying to load the allocator multiple times"）。
  → **Playwright の `chrome-headless-shell` を使う**（`~/Library/Caches/ms-playwright/chromium_headless_shell-*/`）。
  独立して動き、ウィンドウも出ない。scratchpad の `shot.sh` がこれ。
- **ページの一部だけ撮るときは transition も殺すこと。**
  **CSSの優先順位で transition は `!important` より強い。** クリップ表示だと画面外扱いで transition が進まず、
  `[data-reveal]` が opacity:0 のまま固まって**真っ白に写る**。
  `[data-reveal]{transition:none!important}` を併せて入れる。
...
**375pxの「見た目」を撮る方法（2026-08-12 追加）**：`--force-device-scale-factor=2` は効かない（CSS幅は変わらず
出力が2倍になるだけ）。**iframeに入れると中のdocumentは指定どおりの幅になる**ので、検証サーバに
`/frame?w=375&h=20000` のようなエンドポイントを足して、`<iframe src="/?..." width=375>` を1枚だけ置いた
ページを撮る。ウィンドウは520px以上にしておくこと（500pxの下限に引っかかるため）。
...
- セクション余白は `section{padding-block:clamp(72px,11vw,112px)}`。
  **上限は 140px → 112px（2026-08-13 ユーザー承認）**。セクションの区切りがPCで約260px→約224pxになる。
  スマホは `clamp` の下限72pxに張り付くので影響しない。
