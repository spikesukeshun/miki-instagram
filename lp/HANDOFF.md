...
### 保留中の確認事項
- **カルーセル投稿側のCTAスライド**は CLAUDE.md の恒久ルールで「MIKI指名 Instagram限定20%OFF（VIPコースのみ）」固定
  （2026-08-15にユーザー判断で「初回限定」から変更。`review_post.py` の `CTA_REQUIRED_TITLE` が正）。
  LPは「MIKI指名 20%OFF ・ VIPコース(150分/180分)限定」表記。
  **2026-08-16 ユーザー判断で決着：LP側の変更は不要。** どちらも「VIPコース限定」を明示しており
  割引の対象条件は一致しているため、遜色なしと判断された。この項目はクローズ。
