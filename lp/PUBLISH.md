...
### 実機結果（2026-09-13・iPhone / iOS 26.6.1 / Instagram 446.0.0.28.66）

| テスト | 結果 |
|---|---|
| I1 `x-safari-https://` → 中継ページ | D（何も起きない・確認の表示も出ない） |
| I2 `x-safari-https://` → 中継ページ（自動） | D |
| I3 `x-safari-https://` → ig.me 直接 | D |
| I4 `googlechromes://` → 中継ページ | D |

**この端末・バージョンでは、iOS の Instagram アプリ内ブラウザは http(s) 以外のスキームへの遷移を受け付けない。**
`instagram://`（2026-08-14 に確認済み）と同じく `x-safari-https://`・`googlechromes://` も無反応で、
確認のダイアログも出ない（自動記録で blur が検知されなかった）。I3 も D なので、止まっているのは
中継ページではなく「外部ブラウザへ出る」段階。**iOS ではスキームによる脱出はできない。**
Android（`intent://`）は未検証。

次の手は、アプリ内ブラウザで DM ボタンが押されたら「閉じるボタン（×）でプロフィールに戻り
『メッセージ』から送る」よう案内する方式（ユーザー判断待ち）。

⚠ Instagram iOS の UA は末尾に `Safari/604.1` を含む。**`Safari/` の有無ではアプリ内ブラウザを判定できない**
ので、`Instagram` の文字列で判定すること（本番LPの対策コードは `/Instagram|FBAN|FBAV/` で判定しており問題ない）。
