...
- **カルーセル投稿側のCTA文言と不一致**：LPは「MIKI指名 20%OFF ・ VIPコース(150分/180分)限定」、
  投稿側は CLAUDE.md の恒久ルールで「MIKI指名 Instagram限定20%OFF（VIPコースのみ）」。
  2026-08-15にユーザー判断で投稿側の「初回限定」→「Instagram限定」へ変更済み（恒久ルール）。
  **2026-08-16 決着：LP側の変更は不要。** どちらも「VIPコース限定」を明示しており対象条件は一致するため。
  この項目はクローズ済み。
