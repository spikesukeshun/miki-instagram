...
## 10. DM導線（Instagram のアプリ内ブラウザ）

Instagram のプロフィール欄のリンクから来た人は、LP を Instagram の**アプリ内ブラウザ**で見ている。
アプリ内ブラウザはアプリのログイン状態を共有しないため、DM ボタン（`ig.me/m/estmiki`）を押すと
**ログイン画面**になる。

### 本番の対策：案内シート（2026-09-13〜）

アプリ内ブラウザ（UA に `Instagram` / `FBAN` / `FBAV`）で DM ボタンが押されたときだけ、
遷移を止めて下から案内シートを出す。実装は `build_site.py` の `build_iab_fix()`（LP本体は変更なし）。

- 手順：① 画面上の「×」でこのページを閉じる ② @estmiki のプロフィールで「メッセージ」をタップ
- 「相談メッセージの下書きをコピーしてDMを開く」から来た場合だけ、貼り付けの案内を足す
- DM を使わない人向けに「ホットペッパーで予約する」（選択中のコースを引き継ぐ）と「ページに戻る」
- 通常のブラウザ（Safari・Chrome など）では何もしない。ig.me がそのままアプリの DM を開く

⚠ **GA4 の数え方は変わっていない。** DM ボタンを押した時点で従来どおり `cta_dm` と
`generate_lead` が1回ずつ送られ、シートのホットペッパーボタンは通常の `cta_hotpepper` として数えられる。
LP の URL もイベント名も変えていないので、ダッシュボードのLPアクセス分析
（`dashboard/fetch_ga4_data.py`）は修正不要。
⚠ `cta_dm` は「DM ボタンが押された数」で、「DM が届いた数」ではない。アプリ内ブラウザでは
押した後にアプリへ戻って送り直す必要があるので、実際に届く DM はこれより少ない。
⚠ 置き換えた旧対策は「プロフィール（instagram.com/estmiki/）へ飛ばす」だったが、行き先でもログインを求められていた。

### 効かなかった方式（試し直さないこと）

| 方式 | 結果 |
|---|---|
| `instagram://user?username=estmiki` へ渡す | 無反応（2026-08-14） |
| `x-safari-https://` で Safari に出てから ig.me を開く | 無反応・確認の表示も出ない（2026-09-13） |
| `x-safari-https://` で ig.me を直接開く | 同上 |
| `googlechromes://` で Chrome に出る | 同上 |

2026-09-13 に iPhone（iOS 26.6.1 / Instagram 446.0.0.28.66）で、テスト用ページ `/dm-test`・`/dm-open`
を使って確かめた。ig.me を直接開く形も無反応なので、止まっているのは中継ページではなく
「外部ブラウザへ出る」段階。**iOS の Instagram アプリ内ブラウザは http(s) 以外のスキームへの遷移を黙って捨てる。**
Android（`intent://`）は未検証。

テスト用ページは役目を終えたので同日に削除した（ソースはコミット `afd81fe` / `2c3a628` の `lp/extra/` に残っている）。

⚠ Instagram iOS の UA は末尾に `Safari/604.1` を含む。**`Safari/` の有無ではアプリ内ブラウザを判定できない**
ので、`Instagram` の文字列で判定すること。

### 補助ページを置く仕組み（`lp/extra/`）

`build_site.py` の `copy_extra_pages()` は `lp/extra/` のファイルを dist にそのまま置く
（今はディレクトリごと無いので何もしない）。次の場合はビルドが止まる：noindex の無いHTML
（`.htm`・大文字拡張子・コメントアウトした meta も含む）／生成物と同名のファイル／
**先頭が `_` のファイル**（`_redirects` 等は Workers が制御ファイルとして読み、置くだけで
本番LPを含むサイト全体の配信を変えられるため）。
