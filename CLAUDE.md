...
### 最終校閲ルール（必須）
**create_post.py 実行完了後、Claude Codeは必ず以下の校閲を自動実行する：**
- 上記「セッション開始時の確認事項」11項目を全てチェック
- 問題があれば content.json を修正して create_post.py を再実行する
- 問題なければ「校閲完了：〇〇問題なし」と報告して次の投稿へ進む
