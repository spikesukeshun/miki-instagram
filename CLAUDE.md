...
### 背景画像（bg_prompt）
- **全スライド共通**: `no people, no skin exposure, no revealing clothing` を必ず含める
- 特に表紙（cover）は肌露出が生じやすいため、さらに `fully clothed, tasteful` を追加する
- 施術シーンや人物が写る画像は使用禁止（花嫁の背中・肌・体のラインが見えるものも禁止）
...
1. 全スライドの bg_prompt に `no skin exposure, no revealing clothing` が入っているか
